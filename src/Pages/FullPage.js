...
export const FullPage = () => {
  ...
  const style = {
    backgroundImage: `linear-gradient(to right, ${gradients[id - 1]?.colorStart}, ${gradients[id - 1]?.colorEnd})`,
  };
        ...
        <div className="flex-fill d-flex" style={style}>
          <nav className="fixed-top nav">
            ...
            <li className="nav-item">
             ...
             {id > 1 ? <Link className="btn btn-dark text-white nav-link me-2" to={`/gradient/${gradients[id - 2]?.id}`} >
               Precedent
              </Link> : ""}
              
            </li>
             <li className="nav-item">
              {id < 25 ? <Link className="btn btn-dark text-white nav-link me-2" to={`/gradient/${gradients[id]?.id}`}>
               suivant
              </Link> : ""}
            </li>
          </nav>
